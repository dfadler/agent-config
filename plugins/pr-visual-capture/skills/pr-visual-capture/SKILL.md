...
attachment upload, e.g. the `gh-attach-image:gh-attach-image` skill) needs a real file path.
...
(e.g. the `gh-attach-image:gh-attach-image` skill) — don't re-derive
...
save; see `gh-attach-image:gh-attach-image`'s SKILL.md for the full explanation of this
behavior).
