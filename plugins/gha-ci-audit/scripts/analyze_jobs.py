...
def job_duration_min(j: dict[str, Any]) -> float | None:
...
def main() -> None:
    ...
    p.add_argument(
        "--filter", metavar="SUBSTR", help="Only show jobs whose name contains SUBSTR"
    )
    p.add_argument(
        "--top", type=int, default=10, help="Show top N longest jobs (default: 10)"
    )
    p.add_argument(
        "--steps",
        action="store_true",
        help="Show step breakdown for the critical-path job",
    )
    args = p.parse_args()
    ...
    completed_dts: list[datetime] = [
        dt for j in completed if (dt := parse_dt(j["completed_at"])) is not None
    ]
    pipeline_end: datetime = max(completed_dts)
    critical = [
        j
        for j in completed
        if j.get("completed_at")
        and (dt := parse_dt(j["completed_at"])) is not None
        and (pipeline_end - dt).total_seconds() < 30
    ]
    ...
    print(
        f"Total job-min (billable estimate): {total_job_min:.1f}m  (parallelism factor: {total_job_min / wall_clock:.1f}x)"
        if wall_clock
        else ""
    )
    print()
    ...
    for j in sorted(critical, key=lambda x: -(x["_dur"] or 0)):
        ...
        print(
            f"  [{runner_type}] {j['name']}: {j['_dur']:.1f}m  conclusion={j.get('conclusion')}"
        )
    print()
