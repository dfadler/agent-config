...
def main():
    p = argparse.ArgumentParser(description='Analyze workflow run data from gh api JSON.')
    p.add_argument('file', nargs='?', help='JSON file (omit to read from stdin)')
    p.add_argument('--group-by', choices=['conclusion', 'event'], default='conclusion',
    ...
    completed = [r for r in runs if r.get('conclusion') and r['conclusion'] not in ('skipped',)]
    ...
    no_duration = 0
    for r in completed:
        key = (r.get('event') if args.group_by == 'event' else r.get('conclusion')) or 'unknown'
        d = duration_min(r)
        if d is not None:
    ...
    for key, ds in sorted(groups.items(), key=lambda x: -len(x[1])):
        ...
        print(f'  {key}: n={len(ds)}, avg={avg:.1f}m, median={med:.1f}m, stdev={sd:.1f}m')
    ...
    print(f'Duration percentiles (all conclusions):')
    print(f'  p50={p50:.1f}m  p90={p90:.1f}m  p99={p99:.1f}m  max={all_durs[-1]:.1f}m')
    print()

    # Bucket distribution
    thresholds = [('<2m', 0, 2), ('2–5m', 2, 5), ('5–15m', 5, 15), ('15–30m', 15, 30), ('>30m', 30, float('inf'))]
