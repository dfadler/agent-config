...
4. Upload both images and embed them via the `gh-attach-image` skill (`~/.claude/skills/gh-attach-image/`) — never commit screenshots to the repo and never use a Gist for this; the skill uses GitHub's own attachment upload endpoint, which needs neither.
