...
signal, run a `pr-babysit` pass on that PR to catch failures and early review
feedback. For CI escalation order and rerun mechanics, see
`dfadler-agent-config:pr-checks`; for review-comment classification and reply
templates, see `dfadler-agent-config:pr-comments`.
...
with why not. For classifying and replying to all open threads at once, use
the `dfadler-agent-config:pr-comments` skill.
