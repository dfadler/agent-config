...
2. Convert to PNG if the native output isn't already raster (e.g. `qlmanage -t -s 1000 -o <dir> <file.svg>` on macOS), then auto-crop to the non-background bounding box before uploading — a raw thumbnail (especially `qlmanage`, which pads to a square) is often mostly whitespace and unreadable once GitHub scales it down. Look at the cropped result before uploading; don't assume it worked.
